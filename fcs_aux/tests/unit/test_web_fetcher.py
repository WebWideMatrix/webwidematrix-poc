...
def test_pull_from_data_pipes(create_bldgs, update_pipe, data_pipes_batch, home_timeline_response):
    ...
    create_bldgs.assert_has_calls(call(content_type=TWITTER_SOCIAL_POST, flr=target_flr, payloads=ANY))
    update_pipe.assert_called_once_with(data_pipes_batch[0]["_id"], {'latestId': home_timeline_response[1].id})
